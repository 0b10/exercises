...
'''
What I learned from this alternative solution:
    * That by first replacing a substring with a space (instead of an empty string), you can then split it on that
        space, regardless of how many consecutive spaces there are:
            song.replace('WUB', '') => 'ABC'
            song.replace('WUB', ' ') => 'A B C'
            song.replace('WUB', ' ') => 'A   B   C'
        The latter two will result in a clean list when using split.
            'ABC'.split()       => ['ABC']  # Not good.
            'A B C'.split()     => ['A', 'B', 'C']
            'A   B   C'.split() => ['A', 'B', 'C']

    The takeaway is that ''.split() doesn't care how many consecutive separators it encounters - a clean list of
